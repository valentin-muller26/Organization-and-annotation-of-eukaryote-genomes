...
# 0) Inputs
# -----------------
wd <- "C:\\Users\\valen\\Desktop\\anotation"
focal_genome <- "Lu_1"
...
# ========================================================================
# GENES PLOT (stacked bar plot per accession)
# ========================================================================
genes_df <- gene_counts_per_genome %>%
  select(genome, gene_core, gene_accessory, gene_specific) %>%
  rename(
    accession = genome,
    Core = gene_core,
    Accessory = gene_accessory,
    Unique = gene_specific
  )
dev.off()
pdf("genes_plot.pdf", width=8, height=5)
par(mar=c(5,4,4,2)+0.1, xpd=TRUE)
bp <- barplot(t(as.matrix(genes_df[,-1])), beside=FALSE, 
              names.arg=genes_df$accession, 
              col=c("darkgreen","steelblue","red"),
              main="Genes per Accession", ylab="Count")

# Add labels on top of bars
totals <- genes_df$Core + genes_df$Accessory + genes_df$Unique
for(i in 1:nrow(genes_df)) {
  label <- paste0("C:", genes_df$Core[i], ", A:", genes_df$Accessory[i], ", U:", genes_df$Unique[i])
  text(x = bp[i], y = totals[i], labels = label, pos = 3, cex = 0.7, font = 2)
}

legend("bottom", legend=c("Core","Accessory","Unique"),
       fill=c("darkgreen","steelblue","red"), horiz=TRUE, inset=-0.28)
dev.off()
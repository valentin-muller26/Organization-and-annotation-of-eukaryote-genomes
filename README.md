...
This analysis is done locally and the files `HiFiasm_Lu1_primary.fa.mod.EDTA.intact.raw.gff3`, generated by EDTA, and `HiFiasm_Lu1_primary.fa.mod.LTR.raw.fa.rexdb-plant.cls.tsv`, generated by TE Sorter, need to be downloaded from the cluster and placed in the same directory as the `01d-full_length_LTRs_identity.R` script.
...
Before running the quality assesment the longest isoform of gene were to be extracted using the script `06a_extract_longest.sh`
...
BUSCO version 5.4.2 is run on both the longest protein and transcript sequences and the `brassicales_odb10` lineage dataset to asses the completness of our annotation.
...
In this step we used GeneSpace to conduct a small pangenome analysis comparing the Lu-1 acession against TAIR10,Altai-5,Are-6 and Etna-2 acession. This analyis was perform in the following step : 
...
**Step 2 : Running Genepace `08b_genespace.R` and `08c_run_genespace.sh`**

The bash script launch the R script to perform the Genespace analysis consisting of synteny and orthology. The analysis generates a pangenome matrix showing orthogroup assignments across all accessions.
...
This script generate statistic of number of orthogroup and genes in the different category for each acession and for all acession and generate the following visualizations :
